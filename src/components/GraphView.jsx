...
export default function GraphView({onSelectCompany}) {
  ...
  const nodeR = n => {
    if(colorMode==="pagerank" && metrics.pagerank[n.id]!==undefined) return Math.max(6, 4 + metrics.pagerank[n.id]*0.22);
    if(colorMode==="betweenness" && metrics.betweenness[n.id]!==undefined) return Math.max(6, 4 + metrics.betweenness[n.id]*0.22);
  ...
  const handleWheel = useCallback(e => { e.preventDefault(); const dz = e.deltaY > 0 ? 0.9 : 1.1; setZoom(z => Math.max(0.3, Math.min(4, z * dz))); }, []);

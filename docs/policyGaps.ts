...
export interface PolicyGap {
  ...
  shortLabel: string;
  description: string;
  frameworkSource: 'Kauffman' | 'EDA' | 'NSF' | 'SBA' | 'USDA';
  severity: 'critical' | 'moderate' | 'watch';
  // Rectangle in data space (x: 0-10 SME→IDE, y: 0-10 stage)
  x1: number; x2: number;
  y1: number; y2: number;
  color: string;
  dashPattern: string;  // SVG stroke-dasharray
  fillOpacity: number;
  labelPosition: { top: boolean; right: boolean };
}
...
export type GapId = typeof POLICY_GAPS[number]['id'];

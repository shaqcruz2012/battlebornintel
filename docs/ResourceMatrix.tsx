// ─────────────────────────────────────────────────────────────────────
// ResourceMatrix.tsx — Nevada Ecosystem Resource Bubble Chart
//
// Converted from BBI_Ecosystem_Graph.html
// Uses IBM Plex Mono font (add to Next.js layout if not present)
// Requires: npm install (no extra deps — pure DOM/SVG rendering)
// ─────────────────────────────────────────────────────────────────────
...
import {
  EcosystemOrg, PolicyGap,
  coreAll, expandedData,
  COLOR_MAP, STAGE_Y_BANDS, STAGE_NAMES,
} from '@/data/ecosystemOrgs';
import { POLICY_GAPS, GapId } from '@/data/policyGaps';
import styles from './ResourceMatrix.module.css';

// ── Types ────────────────────────────────────────────────────────────
type Dataset  = 'core' | 'expanded';
type Track    = 'ALL' | 'IDE' | 'SME' | 'Hybrid';
type StageNum = null | 0 | 1 | 2 | 3;
...
export default function ResourceMatrix() {
  ...
  const [activeTrack, setTrack]     = useState<Track>('ALL');
  const [activeStage, setStage]     = useState<StageNum>(null);
  const [activeCats, setActiveCats] = useState<Set<string>>(new Set(Object.keys(COLOR_MAP)));
  const [labelsOn, setLabels]       = useState(true);
  const [gapVis, setGapVis]         = useState<Record<GapId, boolean>>(
    Object.fromEntries(POLICY_GAPS.map(g => [g.id, false])) as Record<GapId, boolean>
  );

  // Derived dataset
  const rawData: EcosystemOrg[] = dataset === 'core'
    ? [...coreAll]
    : [...coreAll, ...expandedData];
  ...
  useEffect(() => {
    ...
    const toX = (v: number) => PAD.left + (v / AXIS) * pW;
    const toY = (v: number) => PAD.top  + (1 - v / AXIS) * pH;
    ...
    // ── Bubbles + labels ────────────────────────────────────────
    const placed: { cx: number; cy: number; r: number; abbr: string; color: string; bubble: HTMLElement; vis: boolean; sdim: boolean }[] = [];
    ...
    jit.forEach((d, i) => {
      ...
      bubble.addEventListener('mouseenter', (e) => showTooltip(e as MouseEvent, orig, color));
      bubble.addEventListener('mousemove',  (e) => moveTooltip(e as MouseEvent));
    ...
    const lw = (txt: string) => txt.length * 6 + 10;

    const overlapsB = (lx: number, ly: number, lw_: number, lh: number) => {
    ...
    const overlapsL = (lx: number, ly: number, lw_: number, lh: number) => {
      for (const b of labelBoxes) {
        if (lx < b.x + b.w + 2 && lx + lw_ > b.x - 2 && ly < b.y + b.h + 2 && ly + lh > b.y - 2) return true;
      }
      return false;
    };
    const inBounds = (lx: number, ly: number, lw_: number, lh: number) =>
      lx >= PAD.left && ly >= PAD.top && lx + lw_ <= W - PAD.right && ly + lh <= H - PAD.bottom;
    const scorePos = (lx: number, ly: number, lw_: number, lh: number) => {
  ...
  function renderGaps(
    ...
    POLICY_GAPS.forEach(ov => {
      if (!gapVis[ov.id as GapId]) return;
  ...
  // ── Tooltip helpers ──────────────────────────────────────────────
  function showTooltip(e: MouseEvent, d: EcosystemOrg, color: string) {
    const tt = tooltipRef.current;
    if (!tt) return;
    (tt.querySelector('#tt-name') as HTMLElement).textContent = d.name;
    (tt.querySelector('#tt-name') as HTMLElement).style.color = color;
    (tt.querySelector('#tt-type') as HTMLElement).textContent = d.type;
    (tt.querySelector('#tt-track') as HTMLElement).textContent = \`\${d.track} · \${STAGE_NAMES[d.stageN]}\`;
    (tt.querySelector('#tt-stage') as HTMLElement).textContent = \`\${d.stageN} / 3\`;
    (tt.querySelector('#tt-geo') as HTMLElement).textContent   = d.geo;
    (tt.querySelector('#tt-fund') as HTMLElement).textContent  = d.funding;
    (tt.querySelector('#tt-ind') as HTMLElement).textContent   = d.industry;
    (tt.querySelector('#tt-web') as HTMLElement).textContent   = d.website;
    (tt.querySelector('#tt-web') as HTMLElement).style.color   = color;
    const fill = tt.querySelector('#tt-stage-fill') as HTMLElement;
  ...
  const toggleGap = (id: GapId) => {
    setGapVis(prev => ({ ...prev, [id]: !prev[id] }));
  };

  const setAllGaps = (on: boolean) => {
    setGapVis(Object.fromEntries(POLICY_GAPS.map(g => [g.id, on])) as Record<GapId, boolean>);
  };
    ...
    <div className={styles.wrap}>
      ...
      <div className={styles.controlBar}>
        <div className={styles.ctrlGroup}>
          ...
          {(['core', 'expanded'] as Dataset[]).map(d => (
        ...
        <div className={styles.ctrlGroup}>
          ...
          {(['ALL', 'IDE', 'SME', 'Hybrid'] as Track[]).map(t => (
        ...
        <div className={styles.ctrlGroup}>
          ...
          {([null, 0, 1, 2, 3] as StageNum[]).map(s => (
      ...
      <div className={styles.gapBar}>
        ...
        {POLICY_GAPS.map(g => (
          <button key={g.id}
            ...
            style={{ borderColor: gapVis[g.id as GapId] ? g.color : g.color + '55',
                     color:       gapVis[g.id as GapId] ? g.color : g.color + '77',
                     background:  gapVis[g.id as GapId] ? g.color + '20' : 'transparent' }}
            onClick={() => toggleGap(g.id as GapId)}

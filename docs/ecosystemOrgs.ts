...
export interface EcosystemOrg {
  ...
  abbr: string;
  type: string;
  x: number;        // Kauffman SME→IDE track score (0-10)
  y: number;        // ICP stage score: 0=Concept, 10=Series B+
  size: number;     // Relative reach/impact (2-8)
  cat: string;      // Category key matching COLOR_MAP
  track: 'SME' | 'IDE' | 'Hybrid';
  stageN: 0 | 1 | 2 | 3;  // 0=Concept, 1=Validation, 2=EarlyRev, 3=Growth
  geo: string;
  funding: string;
  industry: string;
  website: string;
  dataset?: 'core' | 'expanded';  // undefined = core+stage3
}
...
export const coreData: EcosystemOrg[] = [
...
export const stage3Data: EcosystemOrg[] = [
...
export const expandedData: EcosystemOrg[] = [
  ...
  // Rural and community-level concept-stage resources — idea to first prototype,
  // serving founders outside Las Vegas/Reno metro areas or in workforce dev
  ...
  // Very top of the stack — programs that only engage companies at $1M+ ARR,
  // Series B rounds, national/federal initiative scale, or export/corp level
...
export const coreAll: EcosystemOrg[] = [...coreData, ...stage3Data];

// Convenience: full expanded dataset
export const allOrgs: EcosystemOrg[] = [...coreData, ...stage3Data, ...expandedData];

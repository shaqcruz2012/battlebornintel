...
import { computeIRS } from '../../engine/irs.js';

export default function CompareView({ viewProps }) {
  const {
    config, data, compareList, isMobile,
  } = viewProps;
  ...
  const VERIFIED_EDGES = data.verifiedEdges || [];
  const EXTERNALS = data.externals || [];
  const ACCELERATORS = data.accelerators || [];
  const ECOSYSTEM_ORGS = data.ecosystemOrgs || [];
          ...
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
            ...
            <tbody>
              {[
                ...
                ...(config?.features?.ssbci !== false ? [{ l:"SSBCI Programs", fn:c=>c.eligible.filter(e=>["bbv","fundnv","1864"].includes(e)).length, num:c=>c.eligible.filter(e=>["bbv","fundnv","1864"].includes(e)).length }] : []),

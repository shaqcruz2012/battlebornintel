...
import { GOLD, DARK, CARD, BORDER, TEXT, MUTED, GREEN, RED, ORANGE } from '../../styles/tokens.js';
...
export default function RadarView({ viewProps }) {
  const {
    config, data, scored, isMobile,
    setSelectedCompany, toggleWatchlist, isWatched,
  } = viewProps;
            ...
            <div key={c.id} onClick={() => setSelectedCompany(c)} style={{ display:"flex", alignItems:"center", gap: isMobile ? 8 : 12, padding: isMobile ? "10px 10px" : "10px 16px", background: idx % 2 === 0 ? DARK : CARD+"80", border:`1px solid ${BORDER}60`, borderRadius:8, cursor:"pointer", transition:"all 0.15s" }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }}>
                  ...
                  {!isMobile && <span style={{ fontSize:9, padding:"1px 6px", background:(sc[c.stage]||MUTED)+"20", borderRadius:4, color:sc[c.stage]||MUTED, fontWeight:600 }}>{sl(c.stage)}</span>}
                </div>
              ...
              {!isMobile && c.estimatedCOD && (
                <div style={{ fontSize:9, color:MUTED, width:50, textAlign:"center" }}>{c.estimatedCOD}</div>
              )}
              {!isMobile && typeof computeRiskScore === "function" && c.riskFactors && (
                <div style={{ fontSize:10, fontWeight:700, width:24, textAlign:"center", color: computeRiskScore(c) >= 50 ? RED : computeRiskScore(c) >= 30 ? ORANGE : MUTED }}>{computeRiskScore(c)}</div>
              )}

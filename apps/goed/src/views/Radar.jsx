...
import { CARD, BORDER, DARK, MUTED, GOLD, GREEN, RED, TEXT, STAGE_COLORS, TRIGGER_CFG, GRADE_COLORS, fadeIn, fmt, stageLabel } from "@bbi/ui-core";
...
export default function Radar({ isMobile, isTablet, setSelectedCompany, setView, fundParam }) {
  ...
  const scored = [...companies].sort((a, b) => (b.irs||0) - (a.irs||0));
    ...
    <div style={fadeIn}>
      <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, 1fr)", gap: isMobile ? 8 : 12, marginBottom:20 }}>
        {[
          ...
          { l:"Avg IRS", v:scored.length ? Math.round(scored.reduce((s,c) => s+(c.irs||0),0)/scored.length) : 0, c:GREEN },

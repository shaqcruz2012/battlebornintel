...
import { CARD, BORDER, DARK, MUTED, GOLD, GREEN, BLUE, PURPLE, TEXT, STAGE_COLORS, fadeIn, fmt, stageLabel } from "@bbi/ui-core";
...
export default function Funds({ isMobile, isTablet, setSelectedCompany, setView }) {
  ...
  if (!funds || !companies) return <div style={{ padding:40, textAlign:"center", color:MUTED }}>Loading funds...</div>;
    ...
    return (
      <div style={{ ...fadeIn, padding:px }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(4,1fr)", gap:10, marginBottom:20 }}>
          <Stat label="Portfolio Cos" value={fundDetail.companies} isMobile={isMobile} />
          <Stat label="Deployed" value={fmt(fundDetail.deployed)} color={GREEN} isMobile={isMobile} />
          {fundDetail.allocated && <Stat label="Utilization" value={`${Math.round(fundDetail.deployed/fundDetail.allocated*100)}%`} color={BLUE} isMobile={isMobile} />}
          {fundDetail.leverage && <Stat label="Leverage" value={`${fundDetail.leverage}x`} color={GOLD} isMobile={isMobile} />}
        </div>
        <div style={{ fontSize:13, fontWeight:700, color:TEXT, marginBottom:10 }}>Eligible Pipeline</div>
        <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
          {allScored.filter(c => c.eligible.includes(fundDetail.id)).sort((a,b) => (b.irs||0) - (a.irs||0)).map(c => (
    ...
    <div style={fadeIn}>
      ...
      <div style={{ display:"grid", gap:10 }}>
        {funds.filter(f => {
          ...
          const avgIRS = portCos.length ? Math.round(portCos.reduce((s,c) => s+(c.irs||0),0)/portCos.length) : 0;
          return (
      ...
      <div style={{ marginTop:20, background:CARD, border:`1px solid ${BORDER}`, borderRadius:10, padding: isMobile ? 14 : 20 }}>
        ...
        {allScored.filter(c=>c.eligible.length>0).sort((a,b)=>(b.irs||0)-(a.irs||0)).slice(0,10).map(c => (
